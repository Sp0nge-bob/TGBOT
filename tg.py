...
def _load_cache_file():
    ...
    if not os.path.exists(CACHE_FILE):
        return # Просто выходим, если файла нет
    ...
    try:
        ...
        # Загружаем неделю из кэша, только если она там есть
        file_wk = data.get("wk_cache")
        if file_wk:
            CURRENT_WK_CACHE = file_wk

        # Мягкая очистка старья
        cutoff = time.time() - (MAX_CACHE_AGE_DAYS * 86400)
        _cache = {k: v for k, v in _cache.items() if v[0] > cutoff}
        
        # ОДИН ЛОГ НА ВСЮ ФУНКЦИЮ
        logger.info(f"📦 Кэш десериализован: {len(_cache)} записей. В памяти wk: {CURRENT_WK_CACHE['wk']}")
...
async def get_current_wk(session: ClientSession = None) -> int:
    global CURRENT_WK_CACHE
    
    # 1. Проверка кэша (чтобы не дергать сайт чаще раза в сутки)
    now = time.time()
    if CURRENT_WK_CACHE["wk"] != 0 and (now - CURRENT_WK_CACHE["ts"] < 86400):
        return CURRENT_WK_CACHE["wk"]

    url = "https://lk.ks.psuti.ru/?mn=2&obj=160"
    
    try:
        # 2. Получаем HTML страницы
        # Используем переданную сессию или создаем временную, если бот еще не проинициализирован
        if session is None:
        ...
        # 3. Ищем ВСЕ упоминания wk=цифры (обычно это 323 и 325)
        all_wk_values = re.findall(r"wk=(\d+)", html)
        
        if not all_wk_values:
            logger.warning("⚠️ Не удалось найти ID недель в HTML коде.")
            return CURRENT_WK_CACHE["wk"]

        weeks = sorted(list(set(map(int, all_wk_values))))

        calculated_wk = 0

        if len(weeks) >= 2:
            calculated_wk = min(weeks) + 1
        else:
            found = weeks[0]
            if "следующая неделя" in html.lower():
                calculated_wk = found - 1
            else:
                calculated_wk = found + 1
        ...
        if calculated_wk != 0:
            ...
            return calculated_wk

    except Exception as e:
        logger.error(f"❌ Критическая ошибка при определении недели: {e}")
    
    return CURRENT_WK_CACHE.get("wk", 0)
